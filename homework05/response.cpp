...
bool Response::checkAndRespond(const string& inWord, ostream& toWhere)
{
    ...
    size_t index = inWord.find(keyword.upper()); //find position of keyword
    if(index != string::npos){ //check to make sure keyword is in input message
        respond(toWhere); //add it to the output stream(toWhere) to make it gives the appropriate response
        return true; 
    }
    return false;
}
