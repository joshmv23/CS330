              ...
              int* color, int* distance)
{
	...
	int n = 0, neighbor = 0; //index at which neighbor is at
        //reset vector back to 0's after each iteration
        reset_vector(dst, cols);
        //using SPMV
        for (int i = 0; i < cols; i++) {
            if (src[i] == 1) {
                for (int j = 0; j < rows; j++) {
                    neighbor = mat_trans[j][i] * src[i];
                    if (neighbor) { //if it's a neighbor, non-zero value update neighbor index and dst[j]
                        n = 1;
                        dst[j] = 1;
                    }
                }
        ...
        for (int i = 0; i < rows; i++) {
            if (dst[i] == 1){ //if vertex found in queue of awaiting nodes
                color[i] = 2;//mark node as visited
                distance[i] = iter; //color it black
            }
            src[i] = dst[i]; //update it with new set vertices after each iteration
            if (color[i] == 2) { //if node is visited set the mat_trans row to all 0's
                for (int j = 0; j < cols; j++) {
                    mat_trans[i][j] = 0;
                }
            }
        }
        if (!n){ //if no neighbors exist anymore than terminate the loop
            done = 1;
        }
        iter++; //increment iter
