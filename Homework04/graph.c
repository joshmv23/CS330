...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    for (int i = 0; i < rows; i++) {
	   ...
	   if(adj_mat[i][j]){ //making sure it's not zero
           	adj_node_t *newNode = create_node(j);
           	add_node(myList, i, newNode);
	   }
	}
    }
    fprintf(stdout, "done\n");
         ...
         int* color, int* distance, int* parent)
{
    ...
    while (size != 0) {
        int u = remove_node(Q); //dequeue, following books algo
        size--;//decrement because of dequeue

        adj_node_t* v = list[u]; //create 'v' from the list

        while(v != NULL) {
            if (color[v->vid] == 0) { //if color == 0, v->vid uses the vertex id to index
               color[v->vid] = 1; //set it to 1
               distance[v->vid] = distance[u] + 1; // update distance
               parent[v->vid] = u; //parent index = u
               adj_node_t *newNode = create_node(v->vid);//create new node to be enqueued
               add_node(Q, 0, newNode);//enqueue the node
               size++;
             }
             v = v->next;//update v
        }
	
        color[u] = 2;//book says to do so
    }
    free_adj_list(Q, rows); //free adj. list
