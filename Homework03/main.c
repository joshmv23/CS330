...
{
    unsigned int* tmpRow = (unsigned int*) malloc(sizeof(unsigned int) * (m + 1));
    unsigned int* tmpCol_ind = (unsigned int*) malloc(sizeof(unsigned int) * nnz);
    double *values = (double*) malloc(sizeof(double) * nnz);
    //histo - histogram
    unsigned int* histo = malloc(sizeof(unsigned int) * m);
    memset(histo, 0, sizeof(unsigned int) * m);
    ...
    //update tmpRow values with histo values
    tmpRow[0] = 0;
    for(int i = 0; i < m; i++) {
        tmpRow[i + 1] = histo[i];
    }
    //rPtr - row ptr
    unsigned int* rPtr = (unsigned int*) malloc(sizeof(unsigned int) * m); //row ptr
    memcpy(rPtr, tmpRow, sizeof(unsigned int) * m); //copy the memory over to rPtr
    for(int i = 0; i < nnz; i++)  {
        tmpCol_ind[rPtr[row_ind[i] - 1]] = col_ind[i] - 1;
        values[rPtr[row_ind[i] - 1]] = val[i]; 
        rPtr[row_ind[i] - 1]++; //increment rPtr
    }
    //reassign the tempoary values, rows, cols, etc back to where it's supposed to go
    *csr_row_ptr = tmpRow;
    *csr_col_ind = tmpCol_ind;
    *csr_vals = values;
    //free any temporary memory I used
    free(rPtr);
    free(histo);
}
...
{
  //initialize result array to all zero values
    for(int i = 0; i < m; i++){
        res[i] = 0.0;
    }
    //spMV
